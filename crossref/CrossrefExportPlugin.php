...
class CrossrefExportPlugin extends DOIPubIdExportPlugin {
    ...
    protected CrossrefPlugin $agencyPlugin;

    public function __construct(CrossrefPlugin $agencyPlugin)
    {
        parent::__construct();
    ...
    {
        $value = $this->agencyPlugin->getSetting($contextId, $name);
        error_log("CrossrefExportPlugin::getSetting - contextId: $contextId, name: $name, value: " . ($name === 'password' ? '***' : ($value ?: 'NULL')));
        return $value;
    }
    ...
    public function exportAndDeposit($context, $objects, $filter, string &$responseMessage, $noValidation = null): bool
    {
        ...
        // thus do not display all errors notifications (for every book),
        // just one general.
        // Warnings occurred when the registration was successful will however be
        // displayed for each book.
        ...
        foreach ($objects as $object) {
            ...
            // export file name example: crossref-20160723-160036-books-1-1.xml
    ...
    public function depositXML($objects, $context, $filename)
    {
        ...
        error_log('=== CrossRef Deposit Debug ===');
        error_log('Context ID: ' . $context->getId());
        error_log('Username: ' . ($username ?: 'NULL/EMPTY'));
        error_log('Password exists: ' . (!empty($password) ? 'YES' : 'NO'));
        error_log('Test Mode: ' . ($this->isTestMode($context) ? 'YES' : 'NO'));
        error_log('API URL: ' . ($this->isTestMode($context) ? static::CROSSREF_API_URL_DEV : static::CROSSREF_API_URL));
        ...
        try {
            ...
            error_log('CrossRef Response Status: ' . $response->getStatusCode());
        } catch (RequestException $e) {
            $returnMessage = $e->getMessage();
            error_log('=== CrossRef Exception ===');
            error_log('Exception Message: ' . $returnMessage);
            if ($e->hasResponse()) {
                $eResponseBody = $e->getResponse()->getBody();
                $eStatusCode = $e->getResponse()->getStatusCode();
                error_log('Response Status Code: ' . $eStatusCode);
                error_log('Response Body: ' . substr($eResponseBody, 0, 500));
                if ($eStatusCode == static::CROSSREF_API_DEPOSIT_ERROR_FROM_CROSSREF) {
                    ...
                    $returnMessage = __('plugins.importexport.common.register.error.mdsError', ['param' => $e->getResponse()->getReasonPhrase()]) . ' (401 Unauthorized). Please check your Crossref username and password.';
